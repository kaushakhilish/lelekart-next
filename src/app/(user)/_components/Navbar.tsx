...
<div className="w-[30%]  flex items-center justify-end gap-6 pr-6">
   ...
   <Link href={'/wishlist'}> <Heart size={20}/></Link>
    ...
    <div className="text-md font-semibold">{user || session.status==="authenticated" ? <UserAvatar img={user.image} name={user.name} />: <Button variant="outline" className="text-md border-2 border-black hover:bg-black hover:text-white font-semibold"><Link  href={'/auth/signin'}>Login</Link></Button>}</div>
    </div>
    </div>
  )
}
